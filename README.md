...
- When prompted to remove compressed files, say **no** to be safe. If some file is corrupted and things go wrong, you'll still have your compressed files to try again later. Notice that if you say **no** the archives will be inside the game directories together with the extracted games, so it will take space. Remove them later if everything goes well.
